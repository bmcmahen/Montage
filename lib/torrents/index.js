...
Download.prototype.onFinished = function(){
  ...
  this.torrent.stop();
  currentDownloads.splice(currentDownloads.indexOf(this), 1);
  var self = this;
  Database.findOne('movies', { _id : this.attributes._id })
    .then(function(doc){
      delete doc.torrent;
...
Download.prototype.stopAndDelete = function(fn){
  if (this.interval) clearInterval(this.interval);
  this.torrent.stop();
  currentDownloads.splice(currentDownloads.indexOf(this), 1);
  Database.remove('movies', { _id : this.attributes._id }).then(function(doc, err){
    fn(err, doc);
  });
};
