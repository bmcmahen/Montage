...
var torrents = require('../torrents').currentDownloads;
...
Methods.add({
  ...
  */
  ...
  deleteVideo: function(json){
    return Q.all([
      Q.nfcall(fs.unlink, json.path),
      db.remove('movies', { _id: json._id })
    ]).then(function(){
      publish.removed('movies', json);
    });
  },

  ignoreVideo: function(json){
    // if it's a torrent, stop the download and delete it.
    if (json.torrent) {
      var thisTorrent = _.find(torrents, function(tor){
        return tor.attributes._id === json._id;
      });
      var deferred = Q.defer();
      if (thisTorrent) {
        thisTorrent.stopAndDelete(function(err, doc){
          if (!err) {
            deferred.resolve();
            publish.removed('movies', json);
          } else {
    ...
    // otherwise, set the 'ignore' attribute to true.
    return db.update('movies', { _id: json._id }, {
      $set: { ignore : true}
    }).then(function(){
      publish.removed('movies', json);
    });
