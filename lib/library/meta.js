...
var promiseThrottler = require('p-throttler');
...
// var throttleTest = promiseThrottler.create(15);
// function testThrottle(){
// 	for (var i = 0; i < 500; i++){
// 		throttleTest.enqueue(function(){
// 			return timer();
// 		})
// 		.then(function(){
// 			console.log('yup');
...
var throttler = promiseThrottler.create(30);
function fetchPosters(list){
	var deferred = Q.defer();
	var sizes = ['w342', 'w45', 'w154'];
	var total = 0;
	list = _.isArray(list) ? list : [list];
	...
	list.forEach(function(movie){
		...
		if (poster) {
			sizes.forEach(function(size){
				...
				throttler
					.enqueue(function(){
						return downloadImage(poster, size);
					})
					.then(function(){
						total--;
						console.log(total);
		...
		if (backdrop){
			...
			throttler
				.enqueue(function(){
					return downloadAndPrepareBackdrop(backdrop);
				})
				.then(function(){
					total--;
					console.log(total);
